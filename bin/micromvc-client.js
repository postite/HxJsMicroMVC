...
ClientMain.prototype = {
	__class__: ClientMain
}
var cx = cx || {}
if(!cx.micromvc) cx.micromvc = {}
if(!cx.micromvc.client) cx.micromvc.client = {}
cx.micromvc.client.IJSController = $hxClasses["cx.micromvc.client.IJSController"] = function() {
};
cx.micromvc.client.IJSController.__name__ = ["cx","micromvc","client","IJSController"];
cx.micromvc.client.IJSController.prototype = {
	__class__: cx.micromvc.client.IJSController
}
var HomeController = $hxClasses["HomeController"] = function() {
	console.log("new HomeController");
};
HomeController.__name__ = ["HomeController"];
HomeController.__interfaces__ = [cx.micromvc.client.IJSController];
HomeController.prototype = {
	__class__: HomeController
}
var ContactController = $hxClasses["ContactController"] = function() {
	js.Lib.alert("The ContactController says Hello world!");
};
ContactController.__name__ = ["ContactController"];
ContactController.__interfaces__ = [cx.micromvc.client.IJSController];
ContactController.prototype = {
	__class__: ContactController
}
...
cx.micromvc.client.JSContext = $hxClasses["cx.micromvc.client.JSContext"] = function(registerControllers) {
	this.controllers = new Hash();
	this.keys = new Array();
	var _g = 0;
	while(_g < registerControllers.length) {
		var cntrl = registerControllers[_g];
		++_g;
...
cx.micromvc.client.JSContext.prototype = {
	...
	,getController: function(uri) {
		...
		while(_g < _g1.length) {
			...
			var r = new EReg(key,"");
			if(r.match(uri)) {
				var clss = this.controllers.get(key);
				var args = cx.micromvc.client.JSContext.getMatchedArray(r);
				args.shift();
				try {
	...
	,registerConroller: function(clss) {
		var metaType = haxe.rtti.Meta.getType(clss);
		var uriStr = StringTools.trim(Std.string(metaType.uri));
		if(uriStr == "null") uriStr = "[/]";
		var uri = HxOverrides.substr(uriStr,1,uriStr.length - 2);
		var className = Type.getClassName(clss);
		this.controllers.set(uri,className);
		this.keys.push(uri);
	}
	,keys: null
	,controllers: null
